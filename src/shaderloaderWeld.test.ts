/**
 * The shaderloader's vertex WELD, executed for real — against EVERY active
 * loader (src/shaderloaderHarness.ts): the frozen 0.6, where the weld moved
 * into the loader, and 0.8, whose core carries it verbatim.
 *
 ...
 * fetch the loader from jsdelivr. Hence this suite runs the REAL loader files
 * against the REAL three, rather than pinning a copy against a copy, and runs
 * the SAME assertions against each version, so 0.8 cannot drift from 0.6.
 *
 * The sandbox is the harness's (a plain-script loader under the `node` vitest
 * env needs `vm`), with `THREE` bound to the actual library so the geometry
 * maths is exercised rather than stubbed.
 */
import { describe, it, expect } from 'vitest';
import * as THREE from 'three';
import { ACTIVE_LOADERS, evalLoader, loaderAvailable, type ActiveLoader } from './shaderloaderHarness';
...
function loadComponent(v: ActiveLoader): Comp {
  const { def } = evalLoader(v, { THREE });
...
function instanceOf(v: ActiveLoader, geom: THREE.BufferGeometry, opts: { primitive?: boolean } = {}): Comp {
  const def = loadComponent(v);
...
for (const v of ACTIVE_LOADERS) {
  const instance = (geom: THREE.BufferGeometry, opts: { primitive?: boolean } = {}) => instanceOf(v, geom, opts);

  describe.skipIf(!loaderAvailable(v))(`shaderloader ${v} — shouldWeld, the three gates`, () => {
   ...
   */
  describe.skipIf(!loaderAvailable(v))(`shaderloader ${v} — weld geometry: welds the box, leaves the sphere alone`, () => {
  ...
  describe.skipIf(!loaderAvailable(v))(`shaderloader ${v} — weld lifecycle: ownership and restoration`, () => {
...
/** The measurement the normals-differ rule rests on, pinned as fact — once, not per loader. */
describe('the weld rule measures three, not a loader', () => {
