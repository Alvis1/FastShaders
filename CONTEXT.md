...
│   │   ├── DrawToolbar.tsx            # Draw-tool cluster (rendered inside NodeEditor's bottom-left canvas bar)
│   │   ├── nodes/
...
**No panel headers**: The "Node View" and "TSL Code View" panel labels have been removed. The code editor uses a tab bar with folder-style tabs at the top. The toolbar shows a "Name:" label before the shader name input, with the EXPORT button after it. The preview panel has a compact top bar (labeled Light / Model / Subd selects, Uniforms toggle) and TWO floating clusters on one baseline over the 3D view: bottom-LEFT (play/pause, bg-color swatch, red ✕ Reset) and bottom-RIGHT (VR, fullscreen ⛶) — the display-target pair sits apart so a mis-aimed click on the everyday controls can't throw the app into fullscreen or open a VR window. The node-editor canvas has no MiniMap (removed 2026-07-30); its cost overlay is pinned top-LEFT and the canvas bar bottom-LEFT.

**SplitPane** ([SplitPane.tsx](src/components/Layout/SplitPane.tsx)): The seam itself is **inert** — dragging happens only on the shared `.fs-grip` tab, so a stray press on the seam can't nudge the layout. Uses `setPointerCapture()` so dragging never loses grip, even when the cursor flies over iframes or other elements that would swallow regular mouse events. Drag math is **delta-from-press**: a press snapshots `{x, y, ratio, cross}` and moves apply travel to the snapshot, so grabbing the grip off-centre never snaps the seam. The column ratio clamps to `[0.05, 0.95]` (the store re-clamps tighter); the cross axis instead has a PIXEL floor — `CROSS_MIN_PANE_PX` (30px) — so the code/preview seam rides up until only the code editor's TSL/Output tab bar remains, stopping AT the bar on any screen height (the store's own floor is a loose 0.01 safety net). The app has ONE two-axis **corner grip**: AppLayout passes `crossRatio`/`onCrossRatioChange` (the code/preview split) to the OUTER column splitter, whose grip then drags X = column split and Y = code/preview split in one gesture; the inner splitter renders `grip={false}` (visible seam, nothing interactive). **Shift locks to the dominant axis** measured from the press point (drawing-app convention — pressing Shift mid-drag snaps the other axis back to its press value). **Touch drags start axis-locked** (no Shift key on a touchscreen, and finger wobble would drift both persisted splits): the first 8px of travel picks the axis, a ≥24px pull on the other axis unlocks to free two-axis movement with the freed axis rebased so nothing jumps. Keyboard: the corner grip takes all four arrows (←/→ column, ↑/↓ code/preview; Shift = coarse); `aria-valuetext` carries both percentages (`aria-valuenow` alone would announce nothing for ↑/↓) and `aria-orientation` is omitted on the two-axis grip. `touchAction: none` prevents browser scroll hijack on touch.

**Shared resize grip** ([styles/controls.css](src/styles/controls.css)): ONE visual for every resize affordance — splitters and the asset bar. A tab growing out of the edge it resizes: outlined in the edge's own color, border removed on the touching side, flat corners there and `--border-radius-md` on the far side, sized from the `--ctl-size` token that also sizes the canvas-bar and preview buttons. Inside is a rotated-square diamond split by a center line into two arrows pointing along the drag axis (a rotated box, not a `◇` glyph — the app self-hosts a woff2 *subset* of Inter, so geometric-shapes codepoints would render as tofu offline). The diamond's center line exists only on the asset bar's single-axis grip — the two-axis corner grip keeps a plain diamond (arrows would lie about half its directions) and `cursor: move`. The asset bar's grip sits 20% in from the right of its edge; the corner grip instead anchors at `--fs-grip-offset` (the code/preview ratio, written INLINE on the divider element — never `:root`, where the per-frame write would invalidate the whole document) plus the seam and half the preview control bar, centring it on that bar; a `min()` ceiling clamps it a half-grip above the asset bar (`--fs-asset-bar-h`, published per registered element by `utils/assetBarHeight.ts` from state AND imperatively mid-drag) so it can never park on the bar's tiles and steal their clicks. Grips sit flush against their containing block's far edge (`right: 0`/`bottom: 0`) under a shared contract: **the containing block IS the seam** (SplitPane's divider; the asset bar's `.content-browser__resizer` spans exactly the 2px border-top), so the grip's opaque fill breaks the seam line at any thickness, including the 12px `pointer: coarse` seam (`--fs-seam` token). Both seams highlight together when the corner grip is hovered/dragged (a `:has()` sibling rule in SplitPane.css). `utils/dragChrome.ts` sets `.fs-dragging` on `<html>` for the length of any grip drag: a `user-select: none` on `<body>` is only *inherited*, so panes declaring their own value (Monaco, inputs, node cards) kept painting selection highlights mid-drag.
...
- **Cost coloring**: NodePreviewCard reads `costColorLow`/`costColorHigh` from the zustand store and passes them to `getCostColor()`/`getCostTextColor()`, ensuring consistent cost-based coloring across editor nodes and the content browser.
...
- **Recolor / rename** — `updateGroupData()` patches `data.color` and `data.label`. Header strip + filled body update live.
- **Frame colors** — `getGroupFrameColors()` (colorUtils) returns an **opaque** body fill and resting border, both the group color mixed into white (full-strength border when selected). Never an alpha tint: the canvas backdrop is user-pickable, so a translucent frame would take its color from whatever sits behind it (and wash out on a dark canvas). Because the fill is solid, `.react-flow__node-group` renders at **z-index −1** so wires merely crossing a frame stay visible — React Flow already elevates edges that touch a member (`getElevatedEdgeZIndex`); this extends the same treatment to pass-through wires while keeping the frame above the canvas grid and below its members.
...
- **Asset bar tab** — [SavedGroupCard.tsx](src/components/NodeEditor/SavedGroupCard.tsx) is a draggable tile that mirrors the in-canvas GroupNode visual (colored header, filled body via the shared `getGroupFrameColors`, member count). Drag it onto the canvas; the drop handler in `NodeEditor.onDrop` reads the `application/fastshaders-saved-group` dataTransfer key and calls `instantiateSavedGroup()`. Hover reveals an X button that calls `deleteSavedGroup()`.
...
- **React Flow surfaces flip via props, not tokens**: the dot-grid color is computed in [NodeEditor.tsx](src/components/NodeEditor/NodeEditor.tsx) from canvas contrast and passed as a prop, since it's an SVG fill rather than CSS-token-driven. (The MiniMap was removed 2026-07-30; its mask was the other prop-flipped surface.)
...
- `fs:rightSplitRatio` — code/preview ratio (seeds/clamps `[0.01, 0.75]` — the low floor lets the seam park at the code editor's tab bar; SplitPane's 30px pixel floor is the real drag bound)
