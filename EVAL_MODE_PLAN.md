...
18. **The thank-you screen never hides a failed transfer.** Its upload outcomes read distinctly — sent automatically / sending… / **could not be sent** / **too large for the server** (a package over 64 MiB, refused before any bytes ship, with its size; item 33) — and both failure branches are styled as a warning that names the remedy: the file is already in Downloads, and the **Download** button (always present, and promoted to the primary green action in exactly these cases) gives another copy. The sent and failed branches were verified in a browser by aborting the endpoint request; the too-large branch (needs a package over 64 MiB, e.g. a ~60 MiB model with the mesh included) is not yet browser-verified. NB when testing this, scope selectors to `.csv-import-modal__panel`: the toolbar behind the dialog has its own "Download app" button, which an unscoped text match also hits.
...
28. **A second entry for the no-cost-feedback arm: `/evalp` ("pointless").** `…/evalp/` is `/eval` with `points=off` forced into the stored launch query — every other parameter (`?task=`, `?budget=`) still works there. It removes EVERY point figure: the cost bar, each node's cost badge, the asset tiles' badges, the Add-node menu's per-row cost, and the shader-settings Total/Budget lines. That is deliberately stronger than `?costbar=off`, which hides the total while leaving per-node prices on screen — a participant can still add those up, so it is not an absence of cost feedback at all (`pointsVisible` therefore implies `costBarVisible`). A separate ENTRY rather than a flag because the condition a participant lands in must not depend on the researcher typing a query string correctly. The suppression is ONE CSS sweep keyed on `data-fs-points="off"` (stamped on `<html>` at boot) for the badges — they are drawn by a dozen components and gating each one would be a dozen chances to miss one — plus React gates for the two figures that are PROSE, where `display: none` would leave a blank row. **Display only**: snapshots keep recording `costPoints` and the package still carries the cost table, because the researcher must be able to price what was built in the arm where the participant could not see the price. Verified in a browser on the live study host: `/evalp` 0 badges / 0 bars, `/eval` 51 badges / 1 bar.
29. **`fs.sferas.lv` is a deploy target** (`npm run deploy:sferas`) — base `/`, its own CSP preview origin, docroot `/var/www/fs/src` (the dedicated `fs` nginx container). NB `/var/www/sferas/src` is a DIFFERENT, shared directory holding a dozen unrelated projects; deploying there would scatter the app across someone else's site. Same hard verification gate as the alvismisjuns script: a clean psftp exit proves nothing about what the site serves.
...
33. **Upload too large is its own outcome (2026-09-11).** A package over `MAX_UPLOAD_BYTES` (64 MiB, the server's `$MAX_BYTES`) used to read "Upload failed.", which blamed the network for a size limit. It is `'too-large'` now, decided before any network I/O (`precheckEvalUpload`, so "Uploading…" never flashes). The thank-you screen names the size in MiB, rounded UP so it never prints at the cap (`formatMiB(…, 'up')`), then the usual hand-it-to-the-researcher instructions. The package contents, the consent and the disclosure are unchanged (still consent-7): it is a delivery message, not a new collection. Also corrected: evalUpload.ts claimed the GitHub Pages upload was CSP-blocked (it is `'self'`; GitHub answers 405) and called alvismisjuns "the study host". The study host is fs.sferas.lv, where the relative URL 404s — a separate server-side fix (CORS on upload.php for the preflight AND the POST, `https://alvismisjuns.lv` in the sferas build's `connect-src`, an absolute `EVAL_UPLOAD_URL`), so until it lands every fs.sferas.lv package reads "Upload failed." and travels by download or mail. Item 15's absolute URL describes the alvismisjuns deploy only.
