...
**STATUS: IMPLEMENTED 2026-08-28** (`src/eval/`, `public/eval/index.html`; browser-verified end-to-end incl. mid-session reload recovery and the full package). A 4-lens adversarial review (25 confirmed findings) reshaped several details — see **§7 Implementation deviations** at the end; where this plan and §7 disagree, §7 is what shipped. Still open: the LV SUS back-translation, the DPO/retention lines in the consent text (marked TODO in `ConsentModal.tsx`), delivery option B, and the pilot (§ Phase 7).
...
4. **SUS questionnaire** — in eval mode the toolbar's red `!` opens the System Usability Scale form (10 items, EN/LV) with a participant field, instead of the FeedbackModal.
