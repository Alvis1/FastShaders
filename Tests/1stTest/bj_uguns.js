...
export default function() {
  ...
  const uv1 = add(vec2(sub(mul(_uv1.x, cos(12.243)), mul(_uv1.y, sin(12.243))), add(mul(_uv1.x, sin(12.243)), mul(_uv1.y, cos(12.243)))), vec2(0.5, 0.5));
  const time1 = time;
  const uv2 = uv();
  const mul1 = mul(time1, 0);
  const mul2 = mul(time1, 6.325);
  const perlinNoise1 = perlinNoise({ position: uv2, scale: 1, balance: 1, contrast: 3.077, color: color(0xFFFFFF), background: color(0xB51A00), seed: mul1 });
  const perlinNoise2 = perlinNoise({ position: uv1, scale: -56.263, balance: 0, contrast: 0.168, color: color(0xFFFFFF), background: color(0x000000), seed: mul2 });
  const mix1 = mix(color1, color2, perlinNoise1);
  const mul3 = mul(perlinNoise2, 37.445);
