...
export default function() {
  const color1 = color(0x1100ff);
  const color2 = color(0x0091ff);
  const time1 = time;
  const float1 = float(0);
  const mul1 = mul(time1, 0.2);
  const mul2 = mul(time1, -0.4);
  const voronoiCells1 = voronoiCells({ scale: 0.3, variation: 0, facet: 0, color: color(0x000000), background: color(0xC0D0FF), seed: mul1 });
