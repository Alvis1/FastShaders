...
export default function() {
  const time1 = time;
  const float1 = float(0.3);
  const color1 = color(0xb5e3da);
  const color2 = color(0x172e35);
  const mul1 = mul(time1, 0.2);
  const mul2 = mul(time1, 0.4);
  const voronoiCells1 = voronoiCells({ scale: 0.1, variation: 0, facet: 0, color: color(0x000000), background: color(0xC0D0FF), seed: mul1 });
  const voronoiCells2 = voronoiCells({ scale: 1.2, variation: 0, facet: 0, color: color(0x000000), background: color(0xC0D0FF), seed: mul2 });
  const mul3 = mul(voronoiCells1, 1.5);
  const remap1 = remap(voronoiCells1, 0.5, 1, 0, 1);
  ...
  return { colorNode: mix1, positionNode: positionLocal.add(normalLocal.mul(mul4)), roughnessNode: float1, alphaTest: 0.5 };
}
