import { add, cos, div, exp, Fn, max, mix, mul, mx_noise_float, positionGeometry, sin, sub, uniform, vec3 } from "three/tsl";
...
const shader = Fn(() => {
  ...
  // Rotate position on XY plane → posLocal
  ...
  // === RING PATTERN ===
  // Original: posLocal * exp(scale-3) * vec3(1/lengths, 4, 1/lengths)
  // Build anisotropic scale vector, each component pre-multiplied
  ...
  // Build scaled position per-component
  ...
  // 3D noise iso-surfaces → natural ring pattern
  ...
  // === FIBER PATTERN ===
  // Original: posLocal * exp(scale-2) * vec3(1, fibersDensity, 1)
  // Y-only stretch for grain-parallel stripes
  const scaleSub2 = sub(scale, 2);
  const fiberE = exp(scaleSub2);
  const fiberScaleY = mul(fiberE, fibersDensity);

  // Octave 0: power=2, scaleMult=1
  ...
  // Octave 1: power=1.2, scaleMult=1.8
  ...
  // Octave 2: power=0.72, scaleMult=3.24
  ...
  // Octave 3: power=0.432, scaleMult=5.832
  ...
  // Accumulate and apply sin banding
