...
export default function() {
  const color1 = color(0x1eebc9);
  const color2 = color(0x06a290);
  const time1 = time;
  const float1 = float(0.1);
  const mul1 = mul(time1, 0.2);
  const mul2 = mul(time1, -0.4);
  const voronoiCells1 = voronoiCells({ scale: 0.2, variation: 0, facet: 0, color: color(0x000000), background: color(0xC0D0FF), seed: mul1 });
  const voronoiCells2 = voronoiCells({ scale: 1, variation: 0.3917, facet: 0, color: color(0x000000), background: color(0xC0D0FF), seed: mul2 });
  const mul3 = mul(voronoiCells1, 1.2);
  const remap1 = remap(voronoiCells1, 0.5, 1, 0, 0.5);
  const add1 = add(voronoiCells1, voronoiCells2);
  const add2 = add(mul3, voronoiCells2);
  const mul4 = mul(add1, 0);
  const add3 = add(remap1, add2);
  const clamp1 = clamp(add3, 0, 1);
  const mix1 = mix(color1, color2, clamp1);

  return { emissiveNode: mix1, positionNode: positionLocal.add(normalLocal.mul(mul4)), roughnessNode: float1 };
}
