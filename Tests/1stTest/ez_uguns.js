...
export default function() {
  const time1 = time;
  const _uv1 = sub(mul(uv(), vec2(8, 1.91)), vec2(0.5, 0.5));
  const uv1 = add(vec2(sub(mul(_uv1.x, cos(-5)), mul(_uv1.y, sin(-5))), add(mul(_uv1.x, sin(-5)), mul(_uv1.y, cos(-5)))), vec2(0.5, 0.5));
  const uv2 = mul(uv(), vec2(2, 1.5));
  const color1 = color(0xfa9200);
  const color2 = color(0xff0000);
  const mul1 = mul(time1, 8);
  const mul2 = mul(time1, -5);
  const perlinNoise1 = perlinNoise({ position: uv1, scale: 1, balance: -0.1, contrast: 0, color: color(0xFFFFFF), background: color(0x000000), seed: mul1 });
  const perlinNoise2 = perlinNoise({ position: uv2, scale: 0, balance: 0, contrast: 0, color: color(0xFFFFFF), background: color(0x000000), seed: mul2 });
