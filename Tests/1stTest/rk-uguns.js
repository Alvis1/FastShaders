...
export default function() {
  const color1 = color(0xd88c01);
  const color2 = color(0xde0101);
  const time1 = time;
  ...
  const perlinNoise2 = perlinNoise({ position: uv2, scale: 0.4, balance: 0, contrast: 0, color: color(0xFFFFFF), background: color(0x000000), seed: mul2 });
  ...
  return { colorNode: mix1, emissiveNode: mix1, positionNode: positionLocal.add(normalLocal.mul(mul5)), opacityNode: mul4, transparent: true };
}
