...
export default function() {
  const color1 = color(0x001eff);
  const color2 = color(0xff5429);
  const _uv1 = sub(mul(uv(), vec2(10, 1)), vec2(0.5, 0.5));
  const uv1 = add(vec2(sub(mul(_uv1.x, cos(-2)), mul(_uv1.y, sin(-2))), add(mul(_uv1.x, sin(-2)), mul(_uv1.y, cos(-2)))), vec2(0.5, 0.5));
  const time1 = time;
  const uv2 = mul(uv(5), vec2(2, 1.5));
  const mul1 = mul(time1, 5);
  const mul2 = mul(time1, -4.4);
  const perlinNoise1 = perlinNoise({ position: uv1, scale: 0, balance: 0, contrast: 0, color: color(0xFFFFFF), background: color(0x000000), seed: mul1 });
  const perlinNoise2 = perlinNoise({ position: uv2, scale: -0.8, balance: 0, contrast: 0, color: color(0xFFFFFF), background: color(0x000000), seed: mul2 });
  const mix1 = mix(color1, color2, perlinNoise1);
  const mul3 = mul(perlinNoise2, -1);
  const add1 = add(mul3, perlinNoise1);
  const mul4 = mul(perlinNoise1, add1);
  const mul5 = mul(mul4, 0.3);
