...
export default function() {
  const color1 = color(0xff9500);
  const color2 = color(0xe60000);
  const time1 = time;
  const _uv1 = sub(mul(uv(), vec2(10, 1.91)), vec2(0.5, 0.5));
  const uv1 = add(vec2(sub(mul(_uv1.x, cos(-5)), mul(_uv1.y, sin(-5))), add(mul(_uv1.x, sin(-5)), mul(_uv1.y, cos(-5)))), vec2(0.5, 0.5));
  const uv2 = mul(uv(), vec2(1.4, 1.5));
  const mul1 = mul(time1, 10);
  const mul2 = mul(time1, -5);
  const perlinNoise1 = perlinNoise({ position: uv1, scale: 1, balance: 0, contrast: 0, color: color(0xFFFFFF), background: color(0x000000), seed: mul1 });
  const perlinNoise2 = perlinNoise({ position: uv2, scale: 0.5, balance: 0, contrast: 0, color: color(0xFFFFFF), background: color(0x000000), seed: mul2 });
  const mix1 = mix(color2, color1, perlinNoise1);
  const mul3 = mul(perlinNoise2, 1.19);
  const add1 = add(mul3, perlinNoise1);
  const mul4 = mul(perlinNoise1, add1);
  const mul5 = mul(mul4, 0.2);

  return { emissiveNode: mix1, positionNode: positionLocal.add(normalLocal.mul(mul5)), opacityNode: mul5, transparent: true };
}
