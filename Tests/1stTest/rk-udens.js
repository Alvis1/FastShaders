...
export default function() {
  const color1 = color(0x0d25cc);
  const color2 = color(0x5791fb);
  const time1 = time;
  const float1 = float(0.1);
  const mul1 = mul(time1, 0.2);
  const mul2 = mul(time1, -0.4);
  const voronoiCells1 = voronoiCells({ scale: 0.5, variation: 0, facet: 0, color: color(0x000000), background: color(0xA6A6A6), seed: mul1 });
  const voronoiCells2 = voronoiCells({ scale: 1.5, variation: 0, facet: 0, color: color(0x000000), background: color(0xDADADA), seed: mul2 });
  const mul3 = mul(voronoiCells1, 1.6);
  const add1 = add(voronoiCells1, voronoiCells2);
  const add2 = add(mul3, voronoiCells2);
  const remap1 = remap(mul3, 0.52, 1, 0, 1);
