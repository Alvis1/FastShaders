...
AFRAME.registerComponent("shader", {
  ...
  applyTSLShader: async function (mesh) {
    ...
    try {
      ...
      const isObjectAPI = hasChannels(shaderResult) || hasParts;
  ...
  storeOriginalMaterials: function (mesh) {
    mesh.traverse((node) => {
      if (node.isMesh && !(node.uuid in this.originalMaterials)) {
        ...
        this.originalMaterials[node.uuid] = m;
      }
    });
  },
  applyMaterialToMesh: function (mesh, material, partMaterials) {
    ...
    mesh.traverse((node) => {
      ...
      let m = material;
      if (partMaterials) {
        const part = partMaterials.get(node.name);
        if (part) {
          m = part;
        } else if (!material) {
  ...
  disposeShaderMaterial: function () {
    ...
    if (this._partMaterials) {
      this._partMaterials.forEach(function (m) {
        try {
          m.dispose();
        } catch (e) {
